...
func (r *resolver) FindClosestDumps(ctx context.Context, repositoryID int, commit, path string, exactPath bool, indexer string) (_ []store.Dump, err error) {
	...
	for _, dump := range candidates {
		...
		if dump.Commit != commit {
			commitExists, err := r.gitserverClient.CommitExists(ctx, dump.RepositoryID, dump.Commit)
			if err != nil {
